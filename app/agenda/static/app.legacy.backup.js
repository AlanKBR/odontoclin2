...
function __initAgendaApp() {
    ...
    Promise.all([
        ...
        try {
            const popWrap = document.getElementById('popover-container');
            if (popWrap) popWrap.innerHTML = popoverHtml;
        } catch (e) { /* noop */ }
        try {
            const ctxWrap = document.getElementById('contextmenu-container');
            if (ctxWrap) ctxWrap.innerHTML = contextHtml;
        } catch (e) { /* noop */ }
        try {
            // Ensure detail popover exists in body to avoid stacking issues
            if (!document.getElementById('eventDetailPopover')) {
                document.body.insertAdjacentHTML('beforeend', detailPopoverHtml);
            } else {
                // If it exists in a wrapper, move it to body
        ...
        // Move context menu to body and hide it explicitly to avoid initial flashing/stacking issues
        try {
        ...
        const calendar = new FullCalendar.Calendar(calendarEl, {
            ...
            events: function (fetchInfo, success, failure) {
                try {
                    ...
                    // Decide coverage: always fetch the full month(s) spanned by the current view
                    let covStart, covEnd;
                    ...
                    if (override && override.start && override.end) {
                        // Use mini calendar's grid range (may include leading/trailing days) to fill cache
                        covStart = startOfDay(new Date(override.start));
                        covEnd = startOfDay(new Date(override.end));
                    } else {
                        // Expand to full months covering the main view's visible span
                        const vs = new Date(fetchInfo.start);
                        const ve = new Date(fetchInfo.end);
                        const monthStart = startOfMonth(vs);
                        const endInc = new Date(ve);
                        endInc.setDate(endInc.getDate() - 1); // make inclusive
                    ...
                    // Use only dates for de-duplication here
                    const dedupKey = `${key}|${covStart.toISOString()}|${covEnd.toISOString()}`;
                    // If cache covers this month coverage, just serve the requested subrange
                    ...
                    const p = fetch(`${BASE}/events?${params.toString()}`)
                        ...
                        .finally(() => {
                            try { pendingEventsFetches.delete(dedupKey); } catch (e) { }
                            // Clear month override after use to prevent redundant refetches
                            try { if (usedOverride) window.__fetchMonthOverride = null; } catch (e) { }
                        });
            ...
            select: function (info) {
                ...
                if (info.allDay) {
                    ...
                    try {
                        // Use Date object to avoid slicing ISO strings that may include seconds/timezone
                        if (info.start instanceof Date && !isNaN(info.start.getTime())) {
                            startInput.value = formatLocalYmdHm(info.start);
                        } else {
                            // Fallback: attempt to parse from string
                            const d = new Date(info.startStr);
                            startInput.value = isNaN(d.getTime()) ? (info.startStr || '').slice(0, 16) : formatLocalYmdHm(d);
                        }
                    } catch (e) {
                        ...
                        fetch(BASE + '/add_event', {
                            ...
                            .then(async (response) => {
                                ...
                                if (response.ok && data.status === 'success' && data.event) {
                                    ...
                                    const msg = (data && data.message) ? data.message : 'Erro ao adicionar evento!';
                                    alert(msg);
                                }
                            })
            ...
            eventClick: function (info) {
                ...
                if (!popover) return; // guard if HTML failed to load
                ...
                // Position like the new-event popover: fixed to mouse, clamped to viewport
                setTimeout(() => {
                    const popRect = popover.getBoundingClientRect();
                    let left = x;
                    let top = y;
            ...
            eventDidMount: function (info) {
                ...
                info.el.addEventListener('contextmenu', function (e) {
                    e.preventDefault();
                    const menu = document.getElementById('eventContextMenu');
                    if (!menu) return; // guard if HTML failed to load
                    let x = e.clientX,
                        y = e.clientY;
        ...
        // Track último ponto do mouse para posicionar popovers próximos ao cursor
        let __lastMousePos = { x: null, y: null };
        try {
            // usar capture para registrar antes de handlers que podem parar propagação
            document.addEventListener('mousedown', (e) => {
                __lastMousePos = { x: e.clientX, y: e.clientY };
            }, true);
        } catch (e) { /* noop */ }

        function computePopupPosition(w, h, anchorEl) {
            let x = (typeof __lastMousePos?.x === 'number') ? __lastMousePos.x : null;
            let y = (typeof __lastMousePos?.y === 'number') ? __lastMousePos.y : null;
            if (x == null || y == null) {
                // fallback: próximo ao botão de âncora
                try {
        ...
        function toggleSettingsMenu(anchorEl) {
            ...
            // Posicionar próximo ao mouse (com fallback ao botão), fixo e com limite da viewport
            setTimeout(() => {
                try {
                    const r = settingsMenu.getBoundingClientRect();
                    const pos = computePopupPosition(r.width, r.height, anchorEl);
                    settingsMenu.style.position = 'fixed';
                    settingsMenu.style.left = pos.left + 'px';
                    settingsMenu.style.top = pos.top + 'px';
        ...
        function toggleSearchMenu(anchorEl) {
            ...
            setTimeout(() => {
                try {
                    const r = searchMenu.getBoundingClientRect();
                    const pos = computePopupPosition(r.width, r.height, anchorEl);
                    searchMenu.style.position = 'fixed';
                    searchMenu.style.left = pos.left + 'px';
                    searchMenu.style.top = pos.top + 'px';
                    searchMenu.style.zIndex = '10000';
                } catch (e) { }
            }, 10);
            // Prefill
            try {
